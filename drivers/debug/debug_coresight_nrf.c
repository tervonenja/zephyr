...
/* Initialize an ATB funnel by nodelabel, only if the node exists in the DTS. */
#define NRF_ATBFUNNEL_INIT(nodelabel)							\
	COND_CODE_1(DT_NODE_EXISTS(DT_NODELABEL(nodelabel)),				\
		(COND_CODE_1(DT_NODE_HAS_PROP(DT_NODELABEL(nodelabel), ctrl_reg),	\
			(nrf_atbfunnel_init(					\
				DT_REG_ADDR(DT_NODELABEL(nodelabel)),		\
				DT_PROP(DT_NODELABEL(nodelabel), ctrl_reg));),	\
			())),							\
		())

static int init_funnels_and_replicators(void)
{
	NRF_ATBFUNNEL_INIT(atbfunnel210)
	NRF_ATBFUNNEL_INIT(atbfunnel211)
	NRF_ATBFUNNEL_INIT(atbfunnel212)
	NRF_ATBFUNNEL_INIT(atbfunnel213)
